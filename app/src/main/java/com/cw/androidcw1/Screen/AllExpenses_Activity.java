...
public class AllExpenses_Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding.fabAddExpense.setOnClickListener(v -> addExpenses());//gán sự kiện click cho floating action button để mở dialog thêm chi tiêu
    ...
    private void addExpenses() {//phương thức mở dialog thêm chi tiêu mới
        ...
        dialogBinding.edtExpenseDate.setOnClickListener(new View.OnClickListener() {//gán sự kiện click cho edit text ngày chi tiêu
            @Override
            public void onClick(View view) {
                ...
                materialDatePicker.addOnPositiveButtonClickListener(selection -> {
                    ...
                    dialogBinding.edtExpenseDate.setText(sdf.format(selection));//set text cho edit text ngày chi tiêu
                });
            }
        });
        dialogBinding.btnAddExpense.setOnClickListener(new View.OnClickListener() {//gán sự kiện click cho button thêm chi tiêu
            @Override
            public void onClick(View view) {
                if (dialogBinding.edtExpenseAmount.getText().toString().isEmpty()) {//nếu edit text số tiền chi tiêu rỗng thì hiển thị thông báo
                    dialogBinding.edtExpenseAmount.setError("Please enter expense amount");//hiển thị thông báo lỗi cho edit text số tiền chi tiêu
                    return;//thoát khỏi phương thức
                }
                if (dialogBinding.edtExpenseDate.getText().toString().isEmpty()) {
                    dialogBinding.edtExpenseDate.setError("Please enter expense description");
                    return;
                }
                ...
                expenses.setAmount(Double.parseDouble(dialogBinding.edtExpenseAmount.getText().toString()));//gán số tiền chi tiêu cho đối tượng expenses
                expenses.setTime(dialogBinding.edtExpenseDate.getText().toString());//gán ngày chi tiêu cho đối tượng expenses
                database_expenes = new database_Expenses(AllExpenses_Activity.this);//khởi tạo database
                if(database_expenes.addExpense(expenses)){//nếu thêm chi tiêu thành công thì hiển thị thông báo
                    ...
                    Toasty.success(getApplicationContext(), "Successfully", Toasty.LENGTH_SHORT).show();
